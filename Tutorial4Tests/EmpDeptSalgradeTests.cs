...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(e => depts.Where(d => d.Loc == "CHICAGO").Select(d => d.DeptNo)
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        var emps = Database.GetEmps();
        var result = emps
            .SelectMany(e => e.Comm.HasValue ? new[] { e } : Array.Empty<Emp>())
            .ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps
            .SelectMany(e => grades.Where(g => e.Sal >= g.Losal && e.Sal <= g.Hisal),
